...
(function () {
    ...
    const SCREEN_CONDITIONS = [
        ...
        { key: 'totalCap', label: '总市值', type: 'realtime', options: [
            ...
            { label: '5000-999999', min: 5000, max: 999999 }
        ...
        { key: 'pe', label: '市盈率PE)', type: 'realtime', options: [
        ...
        { key: 'pb', label: '市净率PB)', type: 'realtime', options: [
    ...
    function jsonp(url, timeout) {
        ...
        return new Promise((resolve, reject) => {
            ...
            script.onerror = () => {
                ...
                finish(new Error('JSONP脚本加载失败（网络或跨域问题）'));
    ...
    async function fetchAllAStocks(onProgress) {
        ...
        const MAX_PAGES = 80; // 最多80页，约4000只股票
        ...
        while (!first || !first.list || first.list.length === 0) {
            ...
            if (firstRetry > 5) {
                ...
                const errMsg = '第一页加载失败，已重试多次。请检查网络或稍后重试';
            ...
            if (firstRetry > 1) {
                ...
                if (loadAllText) loadAllText.textContent = '第一页加载失败，' + firstRetry + '次重试...（已用' + elapsed + 's';
    ...
    async function loadAndDrawKline(code) {
        ...
        if (infoEl) {
            ...
            infoEl.innerHTML =
                ...
                `<span>成交额 ${formatVolume(latest[5])}</span>` +
    ...
    function renderStrategyCards(list) {
        ...
        if (hotSectionTip) {
            ...
            hotSectionTip.textContent = '' + list.length + ' 只符合条件 · 可在搜索框搜索';中进一步筛;
        ...
        if (stockSearch) {
            ...
            stockSearch.placeholder = '' + list.length + ' 只策略结果中搜索（清空恢复热门榜）';热门榜;
    ...
    async function doSearch(keyword) {
        ...
        if (searchResultTitle) searchResultTitle.textContent = '搜索结果'; keyword;
    ...
    function renderScreenResults(results) {
        ...
        table.innerHTML = '<thead><tr><th>名称</th><th>代码</th><th>现价</th><th>涨跌/th><th>PE</th><th>PB</th><th>换手率/th><th>总市值/th><th>操作</th></tr></thead><tbody></tbody>';
    ...
    (async function initHotStocks() {
        ...
        if (hotSectionTip) {
            ...
            const srcInfo = hotStocks.length ? `已加${hotStocks.length} 只热门股 · 来源：东方财同花雪球 · ` : '';
        ...
        btn.addEventListener('click', async () => {
            ...
            if (hotSectionTip) {
                ...
                if (hotStocks.length === 0) {
                    ...
                    hotSectionTip.textContent = `来源{btn.textContent} · ${hotStocks.length} · 点击自选」加入，默认买入100股`;
